    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setTabManageEven: function ()
        {
			...
			$('#tabManage').click(function(){
				var offset = $(this).offset();
				if($('.l-tab-menu').length === 0 ) {
					var menuHtml = '<div class="l-tab-menu">' + 
										'<p id="tabCloseAll" data-opt="closeall"><b></b>关闭全部</p>' +
										'<p id="tabCloseCur" data-opt="closecur"><b></b>关闭当前页</p>' +
										/*'<p id="tabRefAll" data-opt="reloadall"><b></b>刷新全部</p>' +*/
										'<p id="tabRefCur" data-opt="reloadcur"><b></b>刷新当前页</p>'
								   '</div>'
					$('#page-tab').append(menuHtml);
					$('.l-tab-menu').css({ "top": (offset.top + 30) + "px", "left": (offset.left - $('.l-tab-menu').outerWidth()) + $('#tabManage').outerWidth() + "px"});
					$('.l-tab-menu p').each(function(index, element) {
						$(this).click(function(){
							g._menuItemClick( { id: $(this).data('opt')});
							$('.l-tab-menu').hide();
						})
					});
				} else {
					$('.l-tab-menu').css({ "top": (offset.top + 30) + "px", "left": (offset.left - $('.l-tab-menu').outerWidth()) + $('#tabManage').outerWidth() + "px"}).show();
				}
			});

			 $(document).click(function(e){
				  if(!($(e.target).isChildAndSelfOf(".l-tab-menu")||$(e.target).isChildAndSelfOf("#tabManage"))){
					 $(".l-tab-menu").hide(); 
				  }
			 });
        },
        ...
        setTabButton: function ()
        {
            ...
            {
                g.tab.links.append('<div class="l-tab-links-left"><i></i></div><div class="l-tab-links-right"><i></i></div>');
                g.setTabButtonEven();
                return true;
            } else
            {
        ...
        addTabItem: function (options)
        {
			...
			if($('#tabManage').length === 0) {
				g.tab.links.ul.append(tabitem);
				g.tab.links.ul.append('<li id="tabManage"><i></i></li>');				
				g.setTabManageEven();
			} else {
				tabitem.insertBefore("#tabManage");
			};
        ...
        removeTabItem: function (tabid)
        {
            ...
            {
				var _tabid = $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().attr('tabid');
				g.selectTabItem(_tabid);
            ...
            if (jframe.length)
            {
                ...
                navigator.appName === 'Microsoft Internet Explorer' && CollectGarbage();
                jframe.remove();
            } 
            contentItem.remove();
        ...
        {
            var g = this, p = this.options;
			g.actionTabid = g.actionTabid || g.getSelectedTabItemID();
            ...
            switch (item.id)
            {
                case "close":
                    ...
                    break;
                case "closecur":
					if(g.getSelectedTabItemID() === 'index') {
						break;
					}
                    g.removeTabItem(g.getSelectedTabItemID());
                    break;
                ...
                case "reload":
                    ...
                    break;
                case "reloadcur":
                    g.reload(g.getSelectedTabItemID());
                    break;
                case "reloadall":
					var tabidlist = g.getTabidList(null, false);
					$(tabidlist).each(function ()
					{  
						g.reload(this);
					});
