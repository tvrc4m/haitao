...
THISPAGE = {
	...
	loadGrid: function() {
		function a(a, b, c) {
			...
			var d = '<div class="operating" data-id="' + c.id + '"><a class="ui-icon ui-icon-pencil" title="修改"></a></div>';
			return d
		}
		function b(a, b, c) {
	...
	addEvent: function() {
		...
		function(a) {
			...
			parent.tab.addTabItem({
				...
				text: "下单记录",
				url: "./sales.php?id=" + b + "&flag=list" + d
			});
		...
		function(a) {
			if (a.preventDefault(), Business.verifyRight("SO_DELETE")) {
				...
				function() {
					Public.ajaxGet("/scm/invSo.do?action=delete", {
						id: b
					},
					function(a) {
		...
		function(a) {
			a.preventDefault(),
			Business.verifyRight("SO_PRINT") && Public.print({
				title: "购货单列表",
				$grid: $("#grid"),
		...
		function(a) {
			...
			Business.verifyRight("SO_导入") && parent.$.dialog({
				...
				height: 300,
				title: "批量导入",
				content: "url:/sales/import.jsp?type=so",
				lock: !0
			})
		}), $(".wrapper").on("click", "#export",
		function(a) {
			...
			$.dialog({
				...
				button: [{
					...
					callback: function() {
						...
						THISPAGE.reloadData(queryConditions),
						"" !== queryConditions.matchCon && a.$_matchCon.val(queryConditions.matchCon),
						a.$_beginDate.val(queryConditions.beginDate),
						a.$_endDate.val(queryConditions.endDate)
					}
				},
				{
					name: "取消"
				...
				resize: !1,
				content: "url:/sales/sales-search.jsp?type=sales",
