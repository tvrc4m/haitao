...
//给会员充值
if(!empty($_POST["charge"])&&!empty($_POST["cash"])&&is_numeric($_POST["cash"])&&!empty($_POST['email']))
{
	$add_time = time();
	$_POST['email']=trim($_POST['email']);
	$_POST['cash']=$_POST['cash']*1;
	
	$sql="select pay_uid from ".MEMBER." where email='$_POST[email]'";
	$db->query($sql);
	$v=$db->fetchRow();
	//-----------是否开通支付账号
	if($v['pay_uid'])
	{
		...
		$sql = "insert into ".CASHFLOW." 
			(pay_uid,price,time,note,censor,flow_id,statu,type)
			values
			('$v[pay_uid]',$_POST[cash],'$add_time','$note[8]','$_SESSION[ADMIN_USER]','$flow_id','4','1')";
		$db->query($sql);
		//-----------修改金额
		$sql="update ".MEMBER." set cash=cash+".$_POST["cash"]." where email='$_POST[email]'";
		$db->query($sql);
		
		admin_msg('module.php?m=payment&s=member_charge.php','操作成功');
	}
	else
	{
		admin_msg('module.php?m=payment&s=member_charge.php','用户还没有开通支付账户');
	}
}
//----------------------删除流水
if(!empty($_GET['del']))
...
$sqlg="select a.*,b.name,b.email from ".CASHFLOW." a left join ".MEMBER." b on a.pay_uid=b.pay_uid 
...
<div class="bigbox" <?php if($_GET['act']=='add') echo 'style="display:block;"';else echo 'style="display:none;"';?>>
  <div class="bigboxhead tab" style=" width:90%">
	  ...
	  <span class="cbox on"><a href=""><?php echo lang_show('mc');?></a></span>
	  <span class="cbox"><a href="module.php?m=payment&s=recycle_charge.php">回收站</a></span>
  </div>
  <div class="bigboxbody" style="margin-top:-1px;">
    <form name="frmuser" method="POST" action="">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td  class="searh_left">Email</td>
          <td><input class="text" name="email" value="<?php echo $_GET['email']; ?>" type="text" id="uname" size="20" maxlength="20" /></td>
        </tr>
        <tr>
          <td><?php echo lang_show('cmount');?></td>
          <td><input class="text" name="cash" type="text" id="cash" size="10" maxlength="10" onkeyup="check_value();" />
            <span class="bz">正值为加,负值为扣</span></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td><input class="btn" type="submit" name="charge" id="charge" value="<?php echo lang_show('sc');?>" onclick="return confirm('<?php echo lang_show('surec');?>')" /></td>
        </tr>
        <?php 
		  if(!empty($f)&&$f=='t')
		  {
		  ?>
        <tr>
          <td><font color="red"><?php echo lang_show('nou');?></font></td>
        </tr>
        <?php }?>
      </table>
    </form>
...
<div class="bigbox" <?php if($_GET['act']=='') echo 'style="display:block;"';else echo 'style="display:none;"';?>>
  <div class="bigboxhead tab" style="width:90%">
  <span class="cbox on"><a href=""><?php echo lang_show('clist');?></a></span>
