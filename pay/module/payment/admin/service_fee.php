...
//删除
	if($_GET['delid'])
	{
		$sql="delete from ".FEE."  where id='$_GET[delid]'";
		$db->query($sql);
		msg("?m=payment&s=service_fee.php");
	}
	if($_POST['act']=='op')
	{
		if(is_array($_POST['chk']))
		{
			...
			$sql="delete from ".FEE." where id in ($id)";
			$db->query($sql);
		}
		if($_POST['name'])
		{
			foreach($_POST['name'] as $key=>$list)
			{
				...
				if(!empty($list)&&!empty($fee_rates))
				{
					...
					$db->query("update ".FEE." set name='$list',fee_rates='$fee_rates',fee_min='$fee_min',fee_max='$fee_max' where id='$key'");		
				}
			}
		}
		if(!empty($_POST['newname']))
		{
			...
			if(!empty($inserts))
			{
				$sql="insert into ".FEE." (`name`,`fee_rates`,`fee_min`,`fee_max`) values ".implode(",",$inserts);
				$db->query($sql);
			}
		}
		msg("?m=payment&s=service_fee.php");
