...
{
	//---------------------------------------初始化账号
	$sql="select * from ".MEMBER." where pay_email='$_POST[email]'";
	$db->query($sql);
	$re=$db->fetchRow();
	if(empty($re['pay_email']))
	{
		$time=time();
		$sql="insert into ".MEMBER." (userid,real_name,pay_email,regtime,lastLoginTime,mobile_verify,identity_verify) value ('0','administrator','admin@systerm.com','$time','$time','true','true')";
		$db->query($sql);
	}
	//---------------------------------------
	unset($_POST['submit']);
