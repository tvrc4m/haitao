...
$sql="select sum(cash) as num from ".MEMBER;
$db->query($sql);
$all_sum=$db->fetchField('num');

$sql = "select sum(amount) as num from ".CASHPICKUP." where is_succeed = 0 or is_succeed = 10";
$db->query($sql);
$unreachable=$db->fetchField('num');

$sql="select sum(price) as num from ".CASHFLOW." where (statu=2 or statu=3) and mold=3";
$db->query($sql);
$in_cash=$db->fetchField('num');

$sql="select sum(price) as num from ".CASHFLOW." where statu=4 and mold=3";
$db->query($sql);
$sys_cash=$db->fetchField('num');
?>
