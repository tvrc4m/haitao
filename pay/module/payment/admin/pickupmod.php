...
if(isset($_POST["action"])&&!empty($_POST['id'])&&isset($_POST["result"]))
{
	...
	{	
		...
		//------------------更新不可用资金
		$sql = "update ".MEMBER." set unreachable=unreachable-".$_POST['amount']." where pay_uid=$_POST[userid]";
		$db->query($sql);
		//------------------更新流水状态
	...
	{	
		...
		//----------------------增加可用资金
		$sql = "update ".MEMBER." set cash=cash+".$_POST['amount']." where pay_uid=$_POST[userid]";
		$db->query($sql);
		//----------------------更新流水状态为0
...
$db->query("select name,email from ".MEMBER." where pay_uid='$pay_uid'");
$pud = $db->fetchRow();
?>
