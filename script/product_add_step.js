	...
	function showResponse(originalRequest)
	{
		if(originalRequest != "{};")
		{
			...
			for(var k in MyMe)
			{
				var Id=MyMe[k][0];
				var Name=MyMe[k][1];
				a+='<li onclick="district(this);" id="'+Id+'|'+class_div_id+'"><a href="javascript:void(0)"><span class="has_leaf">'+Name+'</span></a></li>';
			}
...
$('#searchBtn').unbind().click(function(){
	$('#searchNone').hide();
	$('#searchSome').hide();
	if($('#SEARCHVALUE').val() == SEARCHVALUE || $('#SEARCHVALUE').val() == '')
	{
		alert(SEARCHVALUE);
	}
	$('.search_result').show();
	...
	function showResponse(originalRequest)
	{
		...
		else
		{
			...
			{
				var Name=MyMe[k];
				tag +=('<li genre="searchList_name" id="'+k+'">'+Name+'</li>');
			}
			$('#searchSome').show();
...
// 返回分类选择
$('a[genre="return_choose_sort"]').unbind().click(function(){
	$('#class_id').val('');
	$('#catid').val('');
	$('#tcatid').val('');
	$('#scatid').val('');
...
// 分类搜索后选择
$('li[genre="searchList_name"]').unbind().click(function(){
	var id = $(this).attr('id')
	$('#class_id').val(id);
	var len=id.length;
	if(len>=4)
