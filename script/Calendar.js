...
{   
   ...
   str += '<div Author="alin" class="cdrWatermark" id="cdrWatermark"></div><div id="cdrBody" style="background:#FFF;position:absolute;left:0px;top:0px;z-index:2;width:160px;">';
   str += this.getHeader();
   str += this.getBody();   
...
Calendar.prototype.getStyle = function()
{
	...
	str += '.calendar{position:absolute;width:160px;height:199px;background-color:'+this.style.bodyBackColor+';border:1px solid ' + this.style.borderColor + ';left:0px;top:0px;overflow:hidden;z-index:'+this.z_index+';}\n';
	str += '.calendar table td{padding:0px;height:auto;line-height:20px;border:none;}\n';
	str += '.cdrHeader{background-color:'+ this.style.headerBackColor +';width:160px;height:22px;font-size:12px;color:'+this.style.headerFontColor+';}\n';
	str += '.cdrWatermark{position:absolute;left:0px;top:95px;width:160px;font-family: Arial Black;font-size:50px;color:'+this.style.watermarkColor+';z-index:1;text-align:center;}\n';
	str += '.cdrBodyBar{background-color:' + this.style.bodyBarBackColor + ';font-size:12px;color:' + this.style.bodyBarFontColor + ';width:160px;height:20px;}\n';
	str += '.cdrBody{width:160px;height:122px;font-size:12px;cursor:pointer;color:' + this.style.bodyFontColor + ';}\n';
	str += '.dayOver{height:16px;padding:0px;border:1px solid black;background-color:#FF0000;color:#FFFFFF;font-size:12px;}\n';
	str += '.dayOut{background-color:#FFF;padding:1px;border:none;height:16px;font-size:12px;}\n';
    str += '.menuOver{background-color:'+this.style.headerBackColor+';color:'+this.style.headerFontColor+';font-size:12px;}\n';
	str += '.headerOver{border:1px solid black;background-color:#f4f4f4;color:black;cursor:default;font-size:12px;}\n';
	str += '.cdrMenu{font-size:12px;border:1px solid #000000;background-color:#ffffff;cursor:default;width:100%;font-size:12px;}\n';
	str += 'html>body #Calendar{font-size:12px;}';
	str += '</style>\n';	
	return str;
...
Calendar.prototype.getHeader = function()
{
	...
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousYear" title="上一年份" style="cursor:pointer;width:20px;" onclick="'+this.Obj+'.onChangeYear(false);"><<</td>\n';
	...
	str += '<td style=" width:40px;" Author="alin" onmouseover="this.className=\'headerOver\'" id="currentMonth" onclick="' + this.Obj + '.showMenu(false);" onmouseout="' + this.Obj + '.hideMenu();this.className=\'\';">0</td>\n';
	...
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextYear" title="下一年份" style="cursor:pointer;width:20px;" onclick="'+this.Obj+'.onChangeYear(true);">>></td></tr>\n';
	str += '</table>\n';
	return str;
};
...
Calendar.prototype.getBody = function()
{
   ...
   str += '</table>\n';
   str += '<table Author="alin" class="cdrBodyBar" cellSpacing="0" cellPadding="0"><tr align="center" Author="alin"><td Author="alin" style="cursor:pointer;" onclick="'+this.Obj+'.getToday();">今天：'+new Date().toFormatString("yyyy年mm月dd日")+'</td></tr></table>\n';
   return str;
};

Calendar.prototype.getBodyBar = function()
{
   var str = '<table Author="alin_bar" id="cdrBodyBar" class="cdrBodyBar" style="cursor:move;" cellSpacing="0" cellPadding="0"><tr Author="alin_bar" align="center">\n';
