...
function iframe_form(id, title, url, width, height, model,fresh) {
    if (!width)	width = 480;
	if (!model) model = 1;
    var d = DialogManager.create(id);
    d.setTitle(title);
    var _html = "<iframe id='iframe_"+id+"' src='" + url + "' width='" + width + "' height='" + height + "' frameborder='0'></iframe>";
    d.setContents(_html);
    d.setWidth(width+2);
    d.setHeight(height + 35);
    d.show('center',model);
...
ScreenLocker = {
    ...
    'lock'      : function(zIndex){
        if (this.masker !== null)
        {
			...
			this.num = this.num + 1;
			//this.masker.css('zIndex', zIndex+102);
			return true;
        }
...
//        }
  ...
  //       });    
		...
		//$(this.dom.wrapper).css('zIndex',1111);
