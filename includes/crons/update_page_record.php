...
else
{
	...
	$onusers=$rs['users']*1;
	//=====================================前一天所有注册的新的会员
	$sql="select count(*) as reguser from ".MEMBER." where TO_DAYS(NOW())-TO_DAYS(regtime)<=1";
	$db->query($sql);
	$rs=$db->fetchRow();
	$nregusers=$rs['reguser']*1;
	//======================================前一天发布产品数
	$sql="select count(*) as pros from ".PRODUCT." where TO_DAYS(NOW())-TO_DAYS(uptime)<=1";
	$db->query($sql);
	$rs=$db->fetchRow();
	$prods=$rs['pros']*1;
	//==========================================以下是写入记录总表
	...
	values
	('$urls','$mostpopurl','$pvs','$ips','$onusers','$nregusers','$prods','0','$ntime')";
	$db->query($sql);
	//==========================================以下是清空详细记录表的记录
	$sql="delete from ".PV;
	$db->query($sql);
