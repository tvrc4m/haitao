...
class admin{
	...
	function is_login($action)
	{
		...
		{
			header("Location: $config[weburl]/login.php?forword=$config[_FORWARD]");
			exit();
		}
		...
		if(!empty($_SESSION["STATU"])&&$_SESSION["STATU"]==1&&$_GET['s']!="admin_member"&&$_GET['s']!="admin_info"&&$_GET['m']!="sns"&&$action!="msg"&&$action!="logout")
		{
	...
	{
		$sql="select a.time,b.user,b.logo,b.userid from ".READREC." a , ".MEMBER." b 
