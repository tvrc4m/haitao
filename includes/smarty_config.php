...
if($config['domaincity']&&!empty($config['baseurl']))
{
	...
	{
		$sql="select con,con2,con3,con4,web_title,web_keyword,web_des,des,copyright,template,logo from ".DOMAIN." where top_domain='http://".$_SERVER['HTTP_HOST']."'";
		$db->query($sql);
		$do=$db->fetchRow();
		if(is_array($do))
		{
			...
			setcookie("SSID",$do['con4'],time()+60*60*24*30,'/');
			
			$dpid=!empty($do['con'])?$do['con']:'';
			$dcid=!empty($do['con2'])?$do['con2']:'';
			$daid=!empty($do['con3'])?$do['con3']:'';
			$dsid=!empty($do['con4'])?$do['con4']:'';
			$config['weburl']="http://".$_SERVER['HTTP_HOST'];
		}
	}
	...
	{
		$sql="select con,con2,con3,con4,web_title,web_keyword,web_des,des,copyright,template,logo from ".DOMAIN." where domain='".$prefix."'";
		$db->query($sql);
		$do=$db->fetchRow();
		if(is_array($do))
		{
...
if(!empty($_COOKIE['CID'])&&empty($dcid))
	$dcid=$_COOKIE['CID'];
if(!empty($_COOKIE['AID'])&&empty($daid))
	$daid=$_COOKIE['AID'];
if(!empty($_COOKIE['SID'])&&empty($dsid))
	$dsid=$_COOKIE['SID'];
$domain = $daid?$daid:($dcid?$dcid:($dpid?$dpid:($dsid?$dsid:"")));
if($config['domaincity'])
$domain = $domain ? $domain : "总站";
//-----------------------------------------
//搜索词入库
if(!empty($_GET['key']))
{
	...
	if (!in_array($_GET[key], $reject_row))
	{
...
if($config['openstatistics'])
{
	...
	{
		global $db;
		$user=empty($_COOKIE['USER'])?NULL:addslashes($_COOKIE['USER']);
