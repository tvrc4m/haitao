...
class home
{
	...
	{
		$sql="select a.time,b.user,b.logo,b.userid from ".READREC." a , ".MEMBER." b where a.userid=b.userid and a.tid='$uid' and a.type='3' limit 0,9";
		$this->db->query($sql);
		$re=$this->db->getRows();
		return $re;
	}
	...
	{
		global $config;
		$sql="select a.id,a.pid,a.commentcount,b.image,b.pname,b.price,b.collectnum,c.sales from ".SPRO." a left join ".SPROINFO." b on a.pid = b.pid left join ".PRODUCT." c on a.pid=c.id where a.uid=$uid and isshare=1 order by a.addtime desc";
	...
	function Blog($uid,$limit=NULL)
	{
		...
		$sql="select a.* , b.member_id as ouid , b.member_name as ouser , b.title as otitle, b.create_time as ocreate_time, b.type as otype, b.img as oimg from ".SNS." a left join ".SNS." b on a.original_id= b.id where a.member_id ='$uid' order by a.create_time desc $str";
		$this->db->query($sql);
		$re=$this->db->getRows();
		foreach($re as $k=>$v)
		{
			$sql="select * from ".SNSCOMMENT." WHERE original_id = '$v[id]' order by id desc";
			$this->db->query($sql);
			$re[$k]['comment']=$this->db->getRows();
			$re[$k]['title']=preg_replace($searcharray,$replacearray,$v['title']);
			$re[$k]['pic']=explode(',',$v['img']);
			$re[$k]['opic']=explode(',',$v['oimg']);
		}
		return $re;
	}
	...
	function UserId()
	{
		$sql="select userid from ".MEMBER." where user='".$_COOKIE['USER']."'";
		$this->db->query($sql);
