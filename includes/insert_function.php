<?php
/**
 * Copyright :Mallbuilder
 * Powered by :Mallbuilder
 * Auther:brad
 * Date: 2008-11-04
 * Des:非公共免费代码，没有得到Mallbuilder许可，禁止传播，复制。
 */
//===================
...
function insert_getUser($ar)
{
	...
	{	
		...
		$sql="select userid,user,logo,$field from ".SHOP." where company!='' and grade>=1 $sql limit 0,".$limit;
		$db->query($sql);
...
function insert_getseach($ar)
{	
	$notvar=$ar['notkey'];
	$va=$ar['va'];
	$urlr="";
	if($va!='')
		$urlr="{$notvar}=".urlencode(trim($va));
	if(is_array($_GET))
	{
		foreach($_GET as $key=>$val)
		{
			if($key!=$notvar&&$val!=''&&$key!='firstRow'&&$key!='totalRows')
				$urlr.=(!empty($urlr)?"&":'')."$key=".urlencode(trim($val));
		}
	}
	return $urlr;
}
