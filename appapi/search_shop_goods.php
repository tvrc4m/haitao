<?php
## 传递参数列表
## shopId【店铺id】,keyWord【关键字】,order【排序】,page【页码】,number【每页数量】
...
if($_POST['shopId']){
    $shopid = $_POST['shopId'];
    $where .=" and member_id=".$_POST['shopId'];
}else{
    $re['result'] = 1;
    echo json_encode($re);
    exit;
...
$or = "";
if($o=='0')
    $or.=" order by clicks desc,id asc";//综合排序（按点击率综合排序）
elseif($o=='1')
    $or.=" order by start_time desc,id asc";//按时间降序排序
elseif($o=='2')
    $or.=" order by price asc,id asc";//按价格由低到高排序
elseif($o=='3')
    $or.=" order by price desc,id asc";//按价格由高到低排序
elseif($o=='4')
    $or.=" order by sales DESC ,id asc ";//按销量排序
