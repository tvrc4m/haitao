        ...
        <tr>
          <td colspan="2" class="td"><?php
			...
			foreach($mem as $key=>$v)
			{
				...
				echo '<div class="clearfix"><label><input '.$str_check.' type="checkbox" name="perm[]" value="'.md5($key).'" />'.$v[0].'</label></div>';
				foreach($v[1] as $ssv)
				{
					
					echo '<ul class="mod clearfix">';
					if($ssv[0])
					{
						echo '<div class="stitle">'.$ssv[0].'</div>';	
					}
					foreach($ssv[1] as $sv)
					{
						$sna=explode(",",$sv);
