...
<div class="bigbox">
	...
	<?php
		...
		{
			$sql="select count(*) as num from ".MEMBER." where invite='".$item['name']."'";
			$db->query($sql);
			$yiji=$db->fetchField('num');//总业绩
			
			$sql="select count(*) as reguser from ".MEMBER." where TO_DAYS(NOW())-TO_DAYS(regtime)=0 and invite='$item[name]'";
			$db->query($sql);
			$rs=$db->fetchRow();
			...
			echo '<tr>
					...
					<td><a href="module.php?m=member&s=member.php&ordrby=lastLoginTime&category=user&Submit=%CB%D1%CB%F7&invite='.$item['name'].'">'.$reguser.'/'.$yiji.'</a></td>
