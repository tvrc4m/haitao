...
if(!empty($_POST['submit'])&&$_POST["submit"]==lang_show('submit'))
{
	...
	{
		$sql="select * from ".CONFIG." where `index`='$pname' and type='wechat'";
		$db->query($sql);
		if($db->num_rows())
			$sql1=" update ".CONFIG." SET value='$pvalue' where `index`='$pname' and type='wechat'";
		else
			$sql1="insert into ".CONFIG." (`index`,value,type) values ('$pname','$pvalue','wechat')";
		$db->query($sql1);
	}
	/****更新缓存文件*********/
	...
	fclose($fp);
	/*********************/
	admin_msg("module.php?m=sms&s=wechat_push_config.php",'设置成功');
}
//===读库函数，生成config形式的数组====
function read_config()
{
	global $db;
	$sql="select * from ".CONFIG." where type='wechat'";
	$db->query($sql);
	$re=$db->getRows();
	foreach($re as $v)
	{
