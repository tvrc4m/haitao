...
if(!empty($_GET['email']))
{
	...
	if($res==1)
		admin_msg("module.php?m=sms&s=mail_config.php",'成功');
	else
		admin_msg("module.php?m=sms&s=mail_config.php",'失败');
}
if(!empty($_POST['submit'])&&$_POST["submit"]==lang_show('submit'))
{
	...
	fclose($fp);
	/*********************/
	admin_msg("module.php?m=sms&s=mail_config.php",'设置成功');
}
//===读库函数，生成config形式的数组====
...
<div class="bigbox" style="margin-top:10px;">
  ...
  <div class="bigboxbody">
    ...
    <input type="hidden" name="s" value="mail_config.php" />
    <input type="hidden" name="m" value="sms" />
      <table width="100%" cellspacing="0">
        <tr>
