<?php
	...
	if(!empty($_POST['chk']))
	{
		...
		{ 
			...
			$db->query($sql);
			$sql="update ".PRODUCT." set promotion_id='0' where id in ($id)";	
			$db->query($sql);
		}
		//审核通过
	...
	//获取产品列表
	$sql="SELECT name as pname,product_id,displayorder,b.id,a.catid,a.price,a.pic,b.member_name,b.status FROM ".ACTIVITYPRODUCT." b left join ".PRODUCT." a on a.id=b.product_id WHERE 1 $str order by b.displayorder,create_time desc";
	...
	$tpl->assign("de",$de);
	$tpl->display("activity_product_list.htm");
