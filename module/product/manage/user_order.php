...
if($_GET['orderstatus']!=''){
    if($_GET['orderstatus']!=5&&$_GET['orderstatus']!=6){
        $subsql.=" and status='".$_GET['orderstatus']."'";
    }else{
        $st = $_GET['orderstatus']-1;
        $sql = "select DISTINCT (order_id) from ".ORPRO." where status=".$st;
        $db->query($sql);
        $od = $db->getRows('order_id');
        $ods="";
        foreach($od as $k=>$o){
            $ods.=$o['order_id'].",";
        }
        $len = strlen($ods)-1;
        $ods = substr($ods, 0, $len);

		if (!$ods)
		{
			$ods = 1;
		}
        $subsql.=" and order_id in (".$ods.")";
    }
}
//buyer_id='0' and
$sql="select * from ".ORDER." where pid='91' and userid!=''  $subsql order by id desc";
//=============================
$page = new Page;
$page->listRows=20;
    ...
    <table width="100%" border="0" cellpadding="2" cellspacing="0">
	 ...
	 ?>
      <tr>
        ...
        <td width="25%" ><?php if($_GET['orderstatus']==5){echo "退款中";}elseif($_GET['orderstatus']==6){echo "退款成功";}else{echo $order_status[$v['status']];}?>&nbsp;</td>
