...
{
	$_POST['rank']*=1;
	$_POST['pname'] = htmlspecialchars($_POST['pname']);
    $_POST['id'] = $_POST['id']*1;
	$sql="update ".PRODUCT." set name='$_POST[pname]',rank='$_POST[rank]' where id='$_POST[id]'";
	$db->query($sql);
...
//===================
$db->query("select * from ".PRODUCT." where id='$_GET[id]' ");
while($db->next_record())
{
	$id=$db->f('id');
	$userid=$db->f('member_id');
	$pname=$db->f('name');
	$rank=$db->f('rank');
}
