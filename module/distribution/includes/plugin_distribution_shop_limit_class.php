...
class distribution_shop_limit
{
	...
	public function update_limit_shop($shop_id,$update_data){
		...
		if($set_str=="")
			return false;
		$sql= sprintf("UPDATE DISTRIBUTION_SHOP_LIMIT SET %s WEHRE shop_id=%s",$set_str,$shop_id);
		$re = $this->db->query($sql);
		return $re;
	}
	public function add_shop_access_user($shop_id,$user_id){
		//判断商铺是否限制分销
		global $db;
		$shop_limit_sql = sprintf("SELECT * FROM DISTRIBUTION_SHOP_LIMIT WHERE shop_id=%s ",$shop_id);
		$shop_limit_re = $this->db->query($shop_limit_sql);
		$shop_limit_result = $this->db->fetchRow();
		if(empty($shop_limit_result)){
			return true;
		}else{
		  $access_user_sql = sprintf("SELECT * FROM DISTRIBUTION_SHOP_ACCESS_USER WHERE shop_id=%s AND user_id=%s",$shop_id,$user_id);
			...
			else
			{
				...
				if($shop_limit_result["status"] ==1){
					...
					if($shop_limit_result["limit_type"]==1){
						$check_sql = sprintf("SELECT COUNT(DISTINCT(op.pid)) AS num FROM ORPRO op, ORDER od WHERE od.order_id=op.order_id AND od.status=4 AND op.pid in (".$shop_limit_result["limit_content"].") AND op.buyer_id=%s AND od.seller_id=%s",$user_id,$shop_id);
						...
						$check_sql = sprintf("SELECT SUM(product_price)  AS total_money FROM ORDER WHERE seller_id=%s AND buyer_id=%s AND status=4",$shop_id,$user_id);
						$check_re = $this->db->query($check_sql);
					...
					//加入分销
					$inser_sql = sprintf("INSERT INTO DISTRIBUTION_SHOP_ACCESS_USER ( `shop_id`, `user_id`, `create_time`) VALUES (%s,%s,%s,%s)",$shop_id,$user_id,time());
	...
	public function is_access_user($product_id,$user_id){
		...
		if(empty($product_id) || empty($user_id))
			return false;
		$product_check_sql = sprintf("SELECT member_id FROM PRODUCT WHERE id=%s",$product_id);
		$this->db->query($product_check_sql)
		$shop_id = $this->db->fetchField("member_id");

		$shop_limit_sql = sprintf("SELECT * FROM DISTRIBUTION_SHOP_LIMIT WHERE shop_id=%s AND status=%s",$shop_id,1);
