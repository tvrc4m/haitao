<?php
	...
	{
		//获取 商品 信息
		$sql="select member_id as userid,member_name as user,price,pic,name as pname from ".PRODUCT." where id='".$_POST['pid']."'";
		$db->query($sql);
		$re=$db->fetchRow();
	...
	if(!empty($_POST['mid'])&&is_numeric($_POST['mid'])&&!empty($_POST['uname'])&&$_POST['op']=="add")
	{
		...
		{			
			...
			$sql="insert into ".FRIEND." (uid,group_id,uname,uimg,fuid,funame,fuimg,addtime,state) VALUES ('$re[userid]','1','$re[user]','$re[logo]','$de[userid]','$de[user]','$de[logo]','".time()."','$state')";
			$db->query($sql);
			die('2');
		}
		else
