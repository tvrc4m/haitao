...
{	
	...
	$new_uid=0;
	$old_uid=0;
	$sql="select userid,user from ".MEMBER." where user IN('$old_user','$new_user')";
	$db->query($sql);
	if($db->num_rows()==2)
	{
	...
	{	
		...
		//---------------------------------
		$sql="delete from ".MEMBER." where userid='$old_uid'";
		$db->query($sql);
		$sql="delete from ".SHOP." where userid='$old_uid'";
		$db->query($sql);
		//=========================
