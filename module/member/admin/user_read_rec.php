...
{
	$sqlk="select userid from ".MEMBER." where user='".$_GET['username']."'";
	$db->query($sqlk);
	$urs=$db->fetchRow();
	if($urs['userid'])
...
$sql="select * from ".READREC." a, ".SHOP." b where a.userid=b.userid $psql";
//------------------------------------
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <?php
	      foreach ($re as $v)
          {
			  $sql="select a.user,b.company from ".MEMBER." a,".SHOP." b  where a.userid=b.userid and a.userid=".$v['userid'];
			  $db->query($sql);
			  $de=$db->fetchRow();
        ?>
        <tr>
          ...
          if ($v['type']==1)
          {
             ...
             $sql="select id,member_id as userid,name as pname from ".PRODUCT." where id=".$v['tid'];
             $db->query($sql);
			 $de=$db->fetchRow();
          ...
          elseif($v['type']==3)
          {
             ...
             $sql="select userid,company from ".SHOP." where userid=".$v['tid'];
             $db->query($sql);
			 $de=$db->fetchRow();
