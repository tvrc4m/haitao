...
if($_POST["action"]=="submit")
{
	...
	$sql="insert into ".NEWSCAT." (cat,pid,ishome,template,pic) values 
	('$_POST[cat]','$_POST[pid]','$ishome','$_POST[template]','$_POST[pic]')";
	$db->query($sql);
}

if($_POST["action"]=="edit")
{
	$sql="update ".NEWSCAT." set cat='$_POST[cat]',pid='$_POST[pid]',ishome='$ishome',template='$_POST[template]',pic='$_POST[pic]'
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td>
			 ...
			 [<a href="javascript:uploadfile('<?php echo lang_show('upload');?>LOGO','pic',85,32)">上传</a>] 
			 [<a href="javascript:preview('pic');">预览</a>]
			 [<a onclick="javascript:$('#pic').val('');" href="#"><?php echo lang_show('del');?></a>]
          </td>
        </tr>
        <tr>
          <td><?php echo lang_show('template');?></td>
          <td><input class="text" type="text" name="template" id="template" value="<?php echo $de['template']; ?>">
          <span class="bz"><?php echo lang_show('templatedes');?></span>
          </td>
        </tr>
      
      
        <tr>
          <td><?php echo lang_show('show');?></td>
          <td><input name="ishome" type="checkbox" class="checkbox" value="1" <?php if(!empty($_GET['ishome'])&&$_GET['ishome']==1) echo "checked"; ?> >
          <span class="bz"><?php echo lang_show('showdes');?></span>
          </td>
        </tr>
        <tr>
          <td>&nbsp;</td>
