<?PHP
  ...
  function get_vote($nid)
  {
  	global $db;
    $sql="select id,title from ".NEWSVOTE." where newsid=$nid order by id asc";	
    $db->query($sql);
	return $db->getRows();
  ...
  {
  	global $db;
    $sql="select catid,cat from ".NEWSCAT." where pid=0 and ishome=1";	
    $db->query($sql);
	$re=$db->getRows();
  ...
  {
	global $db;
	$sql="select name from ".CHA;	
	$db->query($sql);
	return $db->getRows('name');  
  }
  
  function get_pic($id)
  {
	global $db;
	$sql="select pic_url from ".CHA." where id='$id'";	
	$db->query($sql);
	return $db->fetchField('pic_url');  
