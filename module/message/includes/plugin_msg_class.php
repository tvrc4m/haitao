...
class msg
{
	...
	function mail_list($type='inbox')
	{
		...
		{    
		   ...
		   {	
			   $sql="select name,user from ".MEMBER." where userid=".$v['touserid'];
			   $this->db->query($sql);
			   $fc=$this->db->fetchRow();
			   $v["fromname"]=$fc['user'];
			   if($fc['name'])
		   ...
		   {
			   $sql="select name,user from ".MEMBER." where userid=".$v['fromuserid'];
			   $this->db->query($sql);
			   $fc=$this->db->fetchRow();
			   $v["fromname"]=$fc['user'];
			   if($fc['name'])
	...
	function mail_det($id)
	{
		...
		{//收件箱
			$sql="select * from ".MEMBER." where userid='".$re['fromuserid']."'";
			$this->db->query($sql);
			$re["fromInfo"]=$this->db->fetchRow();
		}
		if($re["touserid"]&&$re['msgtype']==2)
		{//发件箱
			$sql="select * from ".MEMBER." where userid='".$re['touserid']."'";
			$this->db->query($sql);
			$re["fromInfo"]=$this->db->fetchRow();
		}
		if($re['contype']==1&&$re['tid'])
		{
			if(substr($re['tid'],strlen($re['tid'])-1,1)==',')
				$re['tid']=$re['tid']."0";
			$sql="select * from ".PRODUCT." where id in ( $re[tid] ) and member_id='$buid'";
			$this->db->query($sql);
			$pro=$this->db->getRows();
			foreach($pro as $v)
			{
		...
		{
			$sql="select * from ".PRODUCT." where id='$re[tid]' and member_id='$buid'";
			$this->db->query($sql);
	...
	{	
		...
		if(!empty($ar))
		{
			$sql="select user,email from ".MEMBER." where userid='$ar[toid]'";
			$this->db->query($sql);
