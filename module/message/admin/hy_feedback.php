...
if(!empty($_GET['from']))
	$psql.=" and fromuserid in (select userid from ".SHOP." where company='$_GET[from]')";
if(!empty($_GET['to']))
	$psql.=" and touserid in (select userid from ".SHOP." where company='$_GET[to]')";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td>
		...
		<input name="s" class="text" type="text" style="display: none;"   value="hy_feedback.php">
...
<table width="100%" border="0" cellpadding="4" cellspacing="0" >
  ...
  <?php 
	for($i=0;$i<count($re);$i++)
	{
		...
		if(!empty($fid))
		{
			$db->query("select user from ".MEMBER." where userid='$fid'");
			$com=$db->fetchRow();
			$fcompany=$com['user'];
			//var_dump($fcompany);exit;
		}
		else
			$fcompany=NULL;
		if(!empty($touserid))
		{
			$db->query("select user from ".MEMBER." where userid='$touserid'");
			$com=$db->fetchRow();
			$tcompany=$com['user'];
		}
		else
  ...
  <tr onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this,'odd')"> 
	...
	?></td>
    <td><div style="overflow:auto; width:100%; height:50px;"> <a style="color:red" >标题：</a><?php echo"$sub<br>";?><a style="color:red">内容：</a><?php echo "$con"; ?></div></td>
    <td>
	<?php
	 if(!empty($fcompany))
