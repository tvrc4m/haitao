...
class credit
{
	...
	{
		global $buid;	
		$sql="select sellerpoints,buyerpoints from ".MEMBER." where userid='$buid' ";
		$this -> db->query($sql);
		$re=$this -> db->fetchRow();	  
		return $re;
				
	}
	function get_user_comment_goodbad_count($type="buyer",$goodbad=NULL,$time=NULL,$sign=NULL,$uid=NULL)
	{
		global $buid;
		if(!empty($uid))
		{
