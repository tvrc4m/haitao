...
class temp
{
	...
	{	
		...
		if($temp=='0')
		{
			$sql="UPDATE ".SHOP." SET template='' WHERE userid='$buid'";
			$this->db->query($sql);
		...
		$sql="UPDATE ".SHOP." SET template='$temp[temp_file]' WHERE userid='$buid'";
		$this->db->query($sql);
	...
	{	
		$sql="select content,product_content as product,title from ".SHOPN." where id=$id";
		$this -> db->query($sql);
		$re=$this -> db->fetchRow();	  
		return $re;
	}
	...
	{			  	
        ...
        $sql="insert into ".SHOPN."(`title` ,`shop_id` ,`content` ,`sort`,`if_show` ,`add_time` ,`url` ,`new_open`,`product_content`)values('$_POST[title]','$buid','$_POST[content]','$_POST[sort]','$_POST[if_show]','".time()."','$_POST[url]','$_POST[new_open]','$product')";
		$this -> db->query($sql);	
		$id=$this -> db->lastid(); 
		return $id;
	}
	...
	function edit_shop_nav($id=NULL)
	{
		...
		if(empty($_POST['title']))
		{
         ...
         $product=implode(",",$_POST['product']);
		$sql="UPDATE ".SHOPN." SET `title` = '$_POST[title]',`content` = '$_POST[content]',`sort` = '$_POST[sort]',`if_show` = '$_POST[if_show]',`url` = '$_POST[url]',`new_open` = '$_POST[new_open]',`product_content`='$product' WHERE  `id` = '$id' and `shop_id` = '$buid' ";
		$this -> db->query($sql);	
		return $id;
	}
	...
	{			  	
		...
		foreach($_POST['prename'] as $key=>$val)
		{
			...
			$v['num']=htmlspecialchars($_POST['prenum'][$key]);
		...
		foreach($_POST['aftername'] as $key=>$val)
		{
			...
			$v['num']=htmlspecialchars($_POST['afternum'][$key]);
			if(!empty($v['name']) and !empty($v['tool']) and !empty($v['num']))
				$inserts[]="('$buid','$v[name]','$v[tool]','$v[num]','$type')";
