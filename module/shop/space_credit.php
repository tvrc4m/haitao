<?php
	...
	foreach($arr as $k=>$goodbad)
	{
		...
		{
			$de[$k][$n]=$credit->get_user_comment_goodbad_count($type,$goodbad,$time,'>',$uid);
		}
		$de[$k][3]=$credit->get_user_comment_goodbad_count($type,$goodbad,$arr_time[2],'<=',$uid);
	}
	$tpl->assign("de",$de);
	$tpl->assign("count",$credit->get_user_comment_goodbad_count($type,'','','',$uid));
