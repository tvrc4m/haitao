...
if(!empty($_POST['user'])&&!isset($_GET['id']))
{
	...
	if(!$db->num_rows())
	{
		...
		$sql="select name from ".DISTRICT." WHERE id='$_POST[street]'";
		$db->query($sql);
		$_POST['street']=$db->fetchField('name');
	
		$sql="insert into ".ADMIN."
		 (user,name,password,group_id,`desc`,province,city,area,street,lang)
		  VALUES
		 ('$_POST[user]','$_POST[name]','$_POST[password]','$_POST[group_id]','$_POST[desc]','$_POST[province]','$_POST[city]','$_POST[area]','$_POST[street]','$_POST[lang]')";
		 $re=$db->query($sql);
		 if($re)
...
if(!empty($_POST['group_id'])&&!empty($_GET['id']))
{
	...
	$sql="update ".ADMIN." set ".$pa." group_id='".$_POST['group_id']."', `desc`='".$_POST['desc']."', province='$_POST[province]', city='$_POST[city]', area='$_POST[area]',street='$_POST[street]',lang='$_POST[lang]',name='$_POST[name]' where id='".$_GET['id']."'";
	$re=$db->query($sql);
	 if($re)
		msg("admin_manager.php");
}
...
<input type="hidden" name="street" id="id_4" value="<?php echo getdistrictid($de["street"]); ?>" />

     
     <?php if($de["province"]){ ?>
        <div id="d_1"><?php echo $de["province"]." ".$de["city"]." ".$de["area"]." ".$de["street"]; ?>&nbsp;&nbsp;<a href="javascript:sd();">编辑</a></div>
        <?php } ?>
