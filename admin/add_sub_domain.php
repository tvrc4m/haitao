...
if(!empty($_POST["Submit"])&&empty($_GET["edit"])&&$add_type==2)
{
	...
	if(!empty($p['province']) and !empty($p['city']))
	{
		...
		$db->query($sql);
		$p['areas']=$db->fetchField('name');
		
		$sql="select name from ".DISTRICT."  WHERE pid='$p[area]'";
		$db->query($sql);
		$re=$db->getRows();
		foreach($re as $v)
		{
			...
			$sql="insert into ".DOMAIN." 
			(`dtype`,`domain`,`con`,`con2`,`con3`,`con4`,`des`,web_title,web_keyword,web_des,copyright,template) 
			values 
			('1','$domain','$p[provinces]','$p[citys]','$p[areas]','$v[name]','$v[des]','$v[web_title]','$v[web_keyword]','$v[web_des]','$v[copyright]','$p[template]')";
			$re=$db->query($sql);
		}
	}
	if($re)
...
if(!empty($_POST["Submit"])&&empty($_GET["edit"])&&$add_type==1)
{
	...
	$sql="select name from ".DISTRICT." WHERE id='$p[street]'";
	$db->query($sql);
	$p['street']=$db->fetchField('name');
	
	$sql="insert into ".DOMAIN." 
	(`dtype`,`domain`,top_domain,`con`,`con2`,`con3`,`con4`,`des`,logo,web_title,web_keyword,web_des,copyright,template) 
	values 
	('1','$p[domain]','$p[top_domain]','$p[province]','$p[city]','$p[area]','$p[street]','$p[des]','$p[logo]','$p[web_title]','$p[web_keyword]','$p[web_des]','$p[copyright]','$p[template]')";
...
if(!empty($_POST["Submit"])&&!empty($_GET["edit"]))
{
	...
	$sql="update ".DOMAIN." set `domain`='$p[domain]',`top_domain`='$p[top_domain]',`con`='$p[province]',`con2`='$p[city]',`con3`='$p[area]',`con4`='$p[street]',`des`='$p[des]',`isopen`='$p[isopen]',web_title='$p[web_title]',web_keyword='$p[web_keyword]',web_des='$p[web_des]',copyright='$p[copyright]',template='$p[template]',logo='$p[logo]' where id='$_GET[edit]'";
	$re=$db->query($sql);
	if($re)
		msg("sub_domain_list.php");
}
...
<input type="hidden" name="street" id="id_4" value="<?php echo getdistrictid($de["con4"]); ?>" />
<?php }?>
     
     <?php if($de["con"]){ ?>
        <div id="d_1"><?php echo $de["con"].$de["con2"].$de["con3"].$de["con4"]; ?>&nbsp;&nbsp;<a href="javascript:sd();">编辑</a></div>
        <?php } ?>
    ...
    <td>
      <input value="<?php if($de['top_domain']){echo $de['top_domain'];}else { echo "http://";}?>" name="top_domain" class="text" type="text" id="top_domain"></td>
    <td></td>
      </tr>
