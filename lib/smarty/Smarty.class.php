 ...
 */
    ...
    function fetch($resource_name, $cache_id = null, $compile_id = null, $display = false)
    {
		...
		{
			if(strpos($this->template_dir,"/pay/"))
				$resource_name="$config[webroot]/pay/admin/templates/".$resource_name;
			else
				$resource_name="$config[webroot]/admin/templates/default/".$resource_name;
		}
		else if(strpos($this->template_dir,"/module/".$_GET['m']."/admin/")&&$config['temp']!='default')
		{
			if(file_exists("$config[webroot]/admin/templates/".$config['temp']."/".$resource_name))
				$resource_name="$config[webroot]/admin/templates/".$config['temp']."/".$resource_name;
		}
		else
		{
			if(!file_exists($this->template_dir.$resource_name)&&$config['language']=='en')
			{
				if(!file_exists("$config[webroot]/templates/en/".$resource_name))
					$resource_name="$config[webroot]/templates/default/".$resource_name;
				else
					$resource_name=	"$config[webroot]/templates/en/".$resource_name;
			}
        ...
        $this->_cache_including = $_cache_including;
			//====================================
				...
				{
					include_once($config['webroot']."/includes/chinese_class.php");
					$_smarty_results=fj($_smarty_results,2);
				}
			//===================================
        if ($display) {
