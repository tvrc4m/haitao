...
{
	define('APP_DIR_NAME', 'mobile_api');
	define('ROOT_PATH', substr(str_replace('\\', '/', dirname(__FILE__)), 0, -19));

	define('LIB_PATH', ROOT_PATH . '/lib');   //ZeroPHP Framework 所在目录
	define('APP_PATH', ROOT_PATH . '/' . APP_DIR_NAME);         //应用程序目录
	define('MOD_PATH', APP_PATH . '/models');       //应用程序模型目录

	global $config;
	$config['webroot'] = ROOT_PATH;
...
define('CODE_TEMPLATE_PATH', ROOT_PATH . '/mobile_api/build_tools/code_template');
define("CONTROLLER_CLASS_NAME", 'Game_'); //控制器class前缀
define("TABEL_PREFIX", 'mallbuilder_'); //控制器class前缀
...
Yf_Registry::set('index_page', 'erp.php');
...
if (CHE || true)
{
	...
	$config_cache['default'] = array(
		'cacheType' => 1,
		'cacheDir' => ROOT_PATH . '/cache/default_cache/',
		'memoryCaching' => false,
...
if (file_exists(ROOT_PATH . '/config/web_config.php'))
{
...
// 	require_once ROOT_PATH . '/config/sms_config.php';
// }

// Yf_Registry::set('sms_config', $sms_config);

// if (file_exists(ROOT_PATH . '/config/logistics_config.php'))
// {
// 	require_once ROOT_PATH . '/config/logistics_config.php';
// }

// Yf_Registry::set('logistics_config', $logistics_config);
?>
