...
class Yf_Model
{
    ...
    {
        $sql = 'SELECT SQL_CALC_FOUND_ROWS ' . $this->_tablePrimaryKey . ', ' . $column . ' FROM ' . $this->_tableName;
        $sql  .= $this->sql->getWhere();
        $rs = $this->sql->getAll($sql);
    ...
    {
        $query = '
                SELECT count(*) as num FROM ' . $this->_tableName;
    ...
    protected function getCacheRow($id=null, &$need_cache_id_name)
    {
        ...
        //fb($rows, "getCacheRow(" . json_encode($id) . ",  " . json_encode($need_cache_id_name) . ")");
        return $rows;
    }
    ...
    public function get($table_primary_key_value=null, $key_row=null)
    {
        ...
        if (is_array($table_primary_key_value))
        {
            ...
            {
                throw new Exception(_('need input （array）table_primary_key_value'));
            }
            ...
            if (CHE && $update_flag)
            {
                if (is_array($table_primary_key_value))
                {
                    foreach($table_primary_key_value as $key => $value)
                    {
                        ...
                        //$this->sql->setPreMemcache(array($this, "removeCache"), array($value));
                    }
                }
                else
                {
                    $this->removeCache($table_primary_key_value);
                    //$this->sql->setPreMemcache(array($this, "removeCache"), array($table_primary_key_value));
                }
            }
